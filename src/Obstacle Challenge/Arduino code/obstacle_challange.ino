...
String p[8] = {"R", "G", "R", "G", "G", "R", "N", "G"};// array for pillars' colors
...
int pid_red_spd = 85; // Red PID speed
...
// passing the green pillar at the starting section
void pass_first_green() {
  if (Distance(1) > 25) // if the robot is far from the left wall turn the robot until the middle ultrasonic sensor detect the parse_val
  {
    while (true)
    {
      if (Parse_Val(arr_left, 3, 3) == 1)
      {
        break;
      }
      forward(drib_spd);
      myservo3.write(pos_L - 20);
    }
    myservo3.write(pos);
    take_ref_mpu();
    ...
    // turn the robot straight again
    while (true)
    {
      if (((theta_mpu - first_ref)) >= -10 )break;
      get_AngleZ();
      theta_mpu = imu_overall_val;
      forward(dribend_spd);
      myservo3.write(pos_L + 35);
    }
    myservo3.write(pos_L);
    referencePID = first_ref;
  }
  // follow with pid algorithm until passing the pillar
  while (true)
  {
    if (dist(2, 3) > 90 || dist(0, 3) > 90 || dist(3, 3) > 90 || dist(5, 3) > 90) break;
    get_AngleZ();
    controlPIDAngle(imu_overall_val);
    forward(pid_spd);
  }
}
// passing the green pillar in the other sections
void pass_green() {
  if (Distance(1) > 25)// if the robot is far from the left wall turn the robot until the middle ultrasonic sensor detect the parse_val
  {
    while (true)
...
// passing the red pillar at the starting section
void pass_first_red()
{
  if (Distance(-1) > 25)// if the robot is far from the right wall turn the robot until the middle ultrasonic sensor detect the parse_val
  {
    while (true)
    {
      if (Parse_Val(arr_right, 3, 3) == 4)
      {
        break;
      }
      forward(drib_spd);
      myservo3.write(pos_R + 20);
    }
    myservo3.write(pos_L);
    ...
    while (true)
    { get_AngleZ();
      if (((imu_overall_val - first_ref)) <= 10 )break;

      forward(dribend_spd);
      myservo3.write(pos_R - 35);
    }
  }
  myservo3.write(pos_R);
  referencePID = first_ref;
  // follow with pid algorithm until passing the pillar
  while (true)
  {
...
// passing the red pillar in the other sections
void pass_red()
{
  if (Distance(-1) > 25)// if the robot is far from the right wall turn the robot until the middle ultrasonic sensor detect the parse_val
  {
...
// controlling the robot when there is no pillars
void pass_none()
{
  referencePID = first_ref;
  // just move forward with pid algorithm
  while (true)
  {
    if (dist(2, 3) > 90 || dist(0, 3) > 90 || dist(3, 3) > 90 || dist(5, 3) > 90) break;
    get_AngleZ();
...
// passing pillars in corner sections
void corner() {
  // if the pillar is second pillar in corner section is green, move backward then turn
  if (p[cnt] == "G")
  {
    while (true)
    {
      if (dist(6, 3) > 80) break;
      backward(spd_back);
      get_AngleZ();
      Back_controlPIDAngle(imu_overall_val);
    }
    stopp();
    delay(5);
    turn();
    take_ref(1);
    first_ref = referencePID;
    cnt++;
    cnt = cnt % 8;
  }
  else if (p[cnt] == "R") // if the pillar is second pillar in corner section is red, move forward then turn
  {
    while (true)
    {
      if (dist(6, 3) < 37) break;
      forward(pid_spd);
      get_AngleZ();
      controlPIDAngle(imu_overall_val);
    }
  ...
  else { // if there is no pillars, turn immediately
    turn();
    take_ref(1);
    first_ref = referencePID;
    cnt++;
...
void turn() {
  ...
  counter += 1;
  take_ref_mpu_ang(91);
  // move forward until passing the corner section
  while (true)
  {
    if ( dist(7, 3) > 110) break;
    if (p[cnt] == "G" && dist(3, 3) < 30)
    { delay(100);
      break;
    }
    if (p[cnt] == "R" && dist(0, 3) < 30) {
      delay(200);
      break;
    }
    forward(spd_after_turn);
    get_AngleZ();
    controlPIDAngle(imu_overall_val);
  }
  stopp();
...
void loop() {
  if (!counter) { // if the robot is at the first section, set straight servo angle and pass the first pillar
    start();
    if (p[cnt] == "G") pass_first_green();
    else if (p[cnt] == "R") pass_first_red();
    else if (p[cnt] == "N") {
      pass_none();
    }
    cnt++;
    cnt = cnt % 8;
    revultra();  // determining the direction of movement
    corner(); // pass the pillars at the corner sections
  }
  // pass the pillars in the straightforward section
  if (p[cnt] == "G") pass_green();
  else if (p[cnt] == "R") pass_red();
  else if (p[cnt] == "N") {
    pass_none();
  }
  cnt++;
  cnt = cnt % 8;
  corner();
  if (counter == 12) // if the three laps are completed, stop the robot
  {
    stopp();
    delay(1000000);
  }
