...
def color_detect(img, width, height, color_name, rectangle_color=(0, 0, 255)):
    ...
    if color_name == 'red': # Red color requires implementing bitwise operation between its two masks to get final mask for it
    ...
    open_img = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel_5, iterations=3)      
    ...
    if color_obj_parameter['n']<1: # no colors where detected
...
def detect_colors(frame):
    out_img = color_detect(frame, 640, 480, 'magenta')
    if not color_obj_parameter['n']:
        magd=False
    else:
        magd=True
    print(magd) #print whether or not the parking lot was detected
    out_img = color_detect(frame, 640, 480, 'red')
    maxr=color_obj_parameter['h']*color_obj_parameter['w']
    out_img = color_detect(frame, 640, 480, 'green')
    maxg=color_obj_parameter['h']*color_obj_parameter['w']
    cv2.imshow('Color detecting ...', out_img)
    if(maxr>maxg):
        print(f"the nearst pillar is red {maxr}")
    else:
        print(f"the nearst pillar is green {maxg}")
